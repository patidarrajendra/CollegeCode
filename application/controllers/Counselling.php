...
class Counselling extends CI_Controller 
{
    ...
    function __construct() 
    {
          ...
          $this->data['title']="College Counselling";
          $this->data['header']=$this->load->view('admin/include/header',$this->data); 
          $this->data['side_bar']=$this->load->view('admin/include/sidebar'); 
    }
    /*Category List*/
    public function counselling_list()
    {
       
        $data['counselling_master'] = $this->common_model->getAlldata('counselling_form','');
        $data['header']=$this->load->view('admin/include/header'); 
        $data['side_bar']=$this->load->view('admin/include/sidebar'); 
        $this->load->view('admin/counselling_list',$data);
        //$data['footer']=$this->load->view('admin/include/footer'); 
    }
    public function view($cust_id = false)
    {
      if(!empty($cust_id))
      {

        $user_data = $this->common_model->getAllRecordsById('counselling_form',array('id' => $cust_id));
        if(empty($user_data))
        {
          $u = base_url()."counselling/counselling_list";
          $this->session->set_flashdata('error', 'User is not available');
          redirect($u);
        }
        $data = array();
        $data['counselling_data'] = $user_data[0];

        $data['header']=$this->load->view('admin/include/header'); 
        $data['side_bar']=$this->load->view('admin/include/sidebar'); 
        $this->load->view('admin/viewstudent_counsell',$data);
      }else
      {
        redirect(base_url()."counselling/counselling_list");
      }
    }
    public function add_category()
    {
        ...
        if($this->input->post()) 
        {
            ...
            else 
            {
                ...
                if($perform_array['category_id']=='')
                {
                    ...
                    $result = $this->common_model->insertData('category_master',$perform_array);
                ...
                else
                {
                      ...
                      $result = $this->common_model->updateFields('category_master',$perform_array,$where);
        ...
        {
            $data['category_master_data'] = $this->common_model->getAlldata('category_master','');
            if($this->uri->segment(3))
            {
               $discount_id=$this->uri->segment(3);
               $select="all";
               $where=array("category_id"=>$discount_id);
               $data['edit_category_data'] = $this->common_model->getAllwherenew_objet('category_master',$where,$select);
               $this->load->view('admin/add_category',$data);
            }
            else
            {
    ...
    public function delete_counselling($id)
    {
        ...
        $where=array("id"=>$id);
        $this->common_model->delete('counselling_form',$where);
        $redirect='counselling/counselling_list';
        redirect($redirect, 'refresh');
    }
