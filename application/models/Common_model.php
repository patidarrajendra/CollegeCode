...
class Common_model extends CI_Model
{
    ...
    function get_selected_field($table, $where=NULL, $fld='', $order_by = '', $order = '')
    {
        if ($fld != NULL) {
            $this->db->select($fld);
        }
    ...
    function getAllwherenew_objet($table, $where, $select = 'all')
    {
        if ($select == 'all') {
            $this->db->select('*');
        } else {
...
//SELECT um.user_id, um.user_fullname, um.user_email FROM sales_order_master as som INNER JOIN sales_order_product AS sop ON sop.sales_order_id=som.sale_order_id  INNER JOIN users_master AS um ON um.user_id=som.user_id WHERE 1
    ...
    function check_oldpassword($oldpass, $user_id)
    {
        ...
        $query = $this->db->get('admin_master'); //data table
        return $query->num_rows();
    }
    ...
    function getSingleRecordpriceById($table,$conditions)
    {
       $this->db->limit(1); 
       $query = $this->db->get_where($table,$conditions);
       return $query->row_array();
...

    {
     ...
     function getDistinctRecordsById($table,$category_id)
   {
       //$this->db->order_by('id', 'desc');
       $sql="SELECT * FROM ".$table." WHERE category_id= ".$category_id." GROUP BY college_id";
    ...
    public function subtotal($table,$where)
    {
    ...
    public function addcartlist($whr)
    {
        //$this->db->select("GROUP_CONCAT(pi.image_name SEPARATOR ', ') image");
        $this->db->select("pcw.*,pm.product_name,pi.image_name");
        $this->db->from("product_cart_to_cart_wishlist as pcw");
        $this->db->join("product_master as pm","pm.product_id = pcw.product_id","left");
        $this->db->join("product_images as pi","pi.product_id = pm.product_id","left","limit",1);
        //$this->db->limit(1);
        $this->db->group_by("pcw.product_cart_id");
        $this->db->where($whr);
    ...
    {
        $this->db->select("pcw.*,pm.product_price");
        $this->db->from("category_catlog_images as pcw");
        $this->db->join("product_master as pm","pm.catlog_id = pcw.catlog_id","left");
        //$this->db->where($whr);
        $this->db->distinct("pm.product_price");
        $result = $this->db->get()->result_array();
        return $result;
    }
    public function getcatlogbycallection($whr)
    {
        $this->db->select("pcw.*,pm.product_price");
        $this->db->from("category_catlog_images as pcw");
        $this->db->join("product_master as pm","pm.catlog_id = pcw.catlog_id","left");
        //$this->db->group_by('pm.product_price');
        $this->db->distinct("pm.product_price");
        $this->db->where($whr);
        $result = $this->db->get()->result_array();
        return $result;
    ...
    //  SELECT id, GROUP_CONCAT(doctor_id SEPARATOR ', ') usernames FROM documents GROUP BY doctor_id

    public function getfirstRecord($table)
    {
        $this->db->limit(1);
    ...
    public function getfirstprice($table,$whr)
    {
        $this->db->limit(1);
        $query = $this->db->get($table);
        $this->db->where($whr);
    ...
    public function getproductdetailwithsupplier($whr)
    {
        $this->db->select("pcw.*,um.user_fname,pi.image_name");
        $this->db->from("product_master as pcw");
        $this->db->join("product_images as pi","pi.product_id = pcw.product_id","left");
        $this->db->join("users_master as um","um.user_id = pcw.suppliers_id","left");
        $this->db->where($whr);
        $result = $this->db->get()->result_array();
        return $result;
    }
